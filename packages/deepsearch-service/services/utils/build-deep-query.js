...
const buildDeepSearchCount = ({ model }) => qb => {
  qb.select(strapi.connections.default.raw(`count (distinct "${model.collectionName}") as "count"`))
}
...
const buildComponentsJoin = ({ qb, model }) => {
  ...
  qb.leftJoin(joinTable, `${joinTable}.${componentsJoinModel.foreignKey}`, `${collectionName}.${primaryKey}`)
...
const buildComponentsTargetJoin = ({ qb, model, joinTable }) => {
  ...
  qb.leftJoin(collectionName, (_qb) => {
    _qb
      ...
      .andOn(`${joinTable}.component_type`, '=', strapi.connections.default.raw(`'${collectionName}'`))
  })
}
